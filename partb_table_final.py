...
for n, n_hidden_units in enumerate(n_hidden_units_l):
    ...
    for (k, (train_index, test_index)) in enumerate(CV.split(X,y)):
        ...
        for (j, (train_index_inner, test_index_inner)) in enumerate(CV.split(X_train_outer,y_train_outer)):
            ...
            if best_lr == None:
                best_lr = learning_curve
            elif mse_inner < error_i[0]:
                best_lr = learning_curve

            torch.save(net, f'model_{n_hidden_units}_{k}_{j}.pth')
            with open('model_performance.txt', 'a') as f:
                f.write(f'{n_hidden_units},{k},{j}' + '\t' + str(mse_inner) + '\t' + str(pcc_inner) + '\t' + str(p_value_inner) + '\n')

        h, = summaries_axes[0].plot(best_lr, color=color_list[k])
        h.set_label('CV fold {0}'.format(k+1))
        summaries_axes[0].set_xlabel('Iterations')
        summaries_axes[0].set_xlim((0, max_iter))
        summaries_axes[0].set_ylabel('Loss')
        summaries_axes[0].set_title('Learning curves for {} hidden neurons'.format(n_hidden_units))
    ...
    summaries_axes[1].bar(np.arange(1, k+2), np.squeeze(np.asarray(errors)), color=color_list)
    summaries_axes[1].set_xlabel('Outer fold')
    summaries_axes[1].set_xticks(np.arange(1, k+1))
    summaries_axes[1].set_ylabel('MSE')
    summaries_axes[1].set_title('Generalization error across outer folds')
    plt.savefig(str(n_hidden_units) + '_H_lcg.png', dpi=800)
    plt.show()

    plt.figure(figsize=(10,10))

    y_est = y_test_est_outer.detach().numpy().T.tolist()[0]; y_true = y_test_est_outer.detach().numpy().T.tolist()[0]
    ...
    plt.grid()
    plt.savefig(str(n_hidden_units) + '_H_fwi.png', dpi=800)
    plt.show()
